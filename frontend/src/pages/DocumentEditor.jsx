...
const Placeholder =
  ...
  PlaceholderModule;
const LinkExtension =
  LinkModule.default || LinkModule.Link || LinkModule;
const TaskList =
...
export default function DocumentEditor() {
  ...
  const resolveFileIdsInHtml = useCallback(async (html) => {
    ...
    try {
      ...
      const imgs = Array.from(
        docNode.querySelectorAll("img[data-file-id]")
      );
  ...
  const normalizeTrailingParagraphsInEditor = useCallback(() => {
    try {
      ...
      const paragraphs = children.filter(
        (ch) =>
          ch.tagName && ch.tagName.toLowerCase() === "p"
      );
  ...
  const ensureCaretVisibleAtTop = useCallback(() => {
    try {
      ...
      if (!scrollParent || scrollParent === document.body) {
        scrollParent =
          editor.view.dom.parentElement || document.body;
      }
      ...
      const desired = Math.max(
        0,
        scrollParent.scrollTop + offsetTop - 12
      );
  ...
  const emitTyping = (isTyping) => {
    ...
    const payload = {
      ...
      socketId:
        (low && low.id) ||
        (sockWrapper && sockWrapper.id) ||
        undefined,
    };

    if (
      sockWrapper &&
      typeof sockWrapper.sendTyping === "function"
    ) {
      try {
  ...
  useEffect(() => {
        ...
        onInit: (data) => {
          if (data?.snapshot !== undefined) {
            ...
            (async () => {
              const html0 = marked(md || "");
              const resolved =
                await resolveFileIdsInHtml(html0);
              try {
    ...
    try {
      ...
      if (!low) {
        console.warn(
          "low-level socket not available from getSocket()"
        );
      } else {
        ...
        if (
          socketRef.current &&
          typeof socketRef.current.joinDoc ===
            "function"
        ) {
        ...
        return () => {
          try {
            if (
              socketRef.current &&
              typeof socketRef.current.leaveDoc ===
                "function"
            )
  ...
  const onFileChange = async (e) => {
    ...
    if (!f) return;
    const workspaceId =
      doc?.workspace?._id || doc?.workspace || null;

    const documentId = id || null;

    try {
      const { file: fileDoc, url } = await uploadFile(f, {
        workspaceId,
        documentId,
      });

      const safeAlt = (
        fileDoc?.originalName || f.name || "image"
      ).replace(/"/g, "");
      const fileId =
      ...
      if (editor && editor.chain) {
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            console.warn(
              "Immediate save after image insert failed",
              e
            );
          }
        }, 260);
  ...
  useEffect(() => {
    ...
    const handler = () => {
      ...
      const prevImgCount =
        (lastSavedMarkdown.current.match(/<img\b/gi) ||
          []).length;
      const newImgCount =
      ...
      if (prevImgCount > 0 && newImgCount < prevImgCount) {
        try {
          const html =
            marked(lastSavedMarkdown.current || "");
      ...
      {showPreviewModal && (
        ...
        >
          ...
          >
            ...
            >
              <small>
                Rendered HTML sanitized (script tags
                removed)
              </small>
            </div>
          </div>
